...
import { PenLine, Trash2, Plus, Calendar } from 'lucide-react';
...
export default function NotesScreen({ onBack, notes, onSave, onDelete }: NotesScreenProps) {
                ...
                {showEditor && (
                    <div className="mb-10 p-8 bg-slate-900/60 backdrop-blur-xl border-2 border-blue-500/30 rounded-[2.5rem] animate-in fade-in zoom-in-95 duration-500 shadow-2xl">
                        ...
                        <div className="flex justify-end gap-4">
                            ...
                            <button
                                onClick={handleSave}
                                className="px-10 py-3 bg-white text-slate-950 text-sm font-bold rounded-2xl hover:bg-blue-50 transition-colors shadow-lg shadow-white/5"
                            >
                                Guardar nota
                            </button>
                        </div>
                        ...
                        <div key={note.id} className="group relative bg-slate-900/40 backdrop-blur-sm border-2 border-slate-800/50 hover:border-blue-500/30 p-8 rounded-[2rem] transition-all hover:translate-y-[-2px] shadow-lg">
                            <div className="flex items-center justify-between mb-5">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] text-blue-400 font-bold uppercase tracking-widest">{note.date}</span>
                                </div>
                                <button
